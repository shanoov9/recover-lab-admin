...
const ClassesPlans = () => {
    ...
    const classesId = params.id;
    const [allClasses, setAllClasses] = useState([]);
    const currentClassName = allClasses.find(cls => cls.id === parseInt(classesId))?.className;
    ...
    const [editYogaPlanDescription, setEditYogaPlanDescription] = useState('');
    const [editYogaPlanPrice, setEditYogaPlanPrice] = useState('');
    const [editYogaPlanDuration, setEditYogaPlanDuration] = useState('');
    ...
    const [editYogaPlanDays, setEditYogaPlanDays] = useState({
        monday: false,
        tuesday: false,
        wednesday: false,
        thursday: false,
    ...
    const handleYogaPlanUpdate = () => {
        ...
        const body = {
            id: editYogaId,
            className: editYogaName,
            classDescription: editYogaPlanDescription,
            classPrice: editYogaPlanPrice,
            classDuration: editYogaPlanDuration,
            classDays: editYogaPlanDays,
        ...
        classApiService.updateClass(body).then(response => {
            ...
            if (response.data.status == true) {
                ...
                setEditYogaPlanPrice('');
                setEditYogaPlanDuration('');
                setEditYogaPlanDescription('');
                    ...
                    <div className='flex gap-2'>
                        ...
                        <button
                            onClick={() => {
                                ...
                                setEditYogaPlanPrice(plan.classPrice);
                                setEditYogaPlanDuration(plan.classDuration);
                                setEditYogaPlanDescription(plan.classDescription);
                                setEditYogaPlanBatchSize(plan.batchSize);
                                setEditYogaPlanDays(JSON.parse(plan.classDays));
        ...
        <div className='mt-4'>
            <div className='mb-3'>
                ...
                <input
                    ...
                    required
                    value={editYogaName}
                    onChange={(e) => { setEditYogaName(e.target.value) }}
                    className='mt-[2px] block w-full px-3 py-1.5 border-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm focus:outline-none'
                />
            </div>
            <div className="flex items-center gap-3">
                <div className='mb-3 w-1/2'>
                    ...
                    <input
                        ...
                        id='planPrice'
                        required
                        value={editYogaPlanPrice}
                        onChange={(e) => { setEditYogaPlanPrice(e.target.value.replace(/[^0-9]/, '')) }}
                        className='mt-[2px] block w-full px-3 py-1.5 border-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm focus:outline-none'
                    />
                </div>
                <div className='mb-3 w-1/2'>
                    ...
                    <div className='relative'>
                        <input
                            ...
                            id='planDuration'
                            required
                            value={editYogaPlanDuration}
                            onChange={(e) => { setEditYogaPlanDuration(e.target.value.replace(/[^0-9]/, '')) }}
                            className='mt-[2px] block w-full px-3 py-1.5 pr-14 border-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm focus:outline-none'
                        />
                        <span className='absolute top-1 right-4 text-gray-700'>
                            Mins.
            ...
            <div className='mb-3 w-full'>
                ...
                <ReactQuill
                    theme="snow"
                    modules={quillModules}
                    value={editYogaPlanDescription}
                    onChange={(value) => { setEditYogaPlanDescription(value) }}
                    className='mt-[2px] block w-full border-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm focus:outline-none'
                />
            </div>
            <div className='mb-3'>
            ...
            <div className="mb-3">
                <label htmlFor='days' className='block text-sm font-medium text-gray-700'>Days</label>
                <div className="flex items-center gap-1 mt-2">
                    {Object.keys(editYogaPlanDays).map((day) => (
                        <div key={day} className='select-none'>
                            <label
                                htmlFor={`day_${day}`}
                                className={` flex items-center gap-0.5 cursor-pointer ${editYogaPlanDays[day] ? 'bg-indigo-500 text-white' : 'bg-gray-300 text-gray-700'} px-3 py-1.5 rounded-md shadow-sm focus:outline-none`}
                            >
                                {
                                    editYogaPlanDays[day] ?
                                        <MinusIcon className='w-4 h-4 inline-block' />
                                        :
                                        <PlusIcon className='w-4 h-4 inline-block' />
                                }
                            ...
                            <input
                                ...
                                name={day}
                                id={`day_${day}`}
                                checked={editYogaPlanDays[day]}
                                onChange={(e) => {
                                    if (e.target.checked) {
                                        setEditYogaPlanDays({ ...editYogaPlanDays, [day]: true });
                                    } else {
                                        setEditYogaPlanDays({ ...editYogaPlanDays, [day]: false });
                                    }
                                }
                                }
                                className='hidden'
    ...
    return (
        <div className='w-full p-5'>
            ...
            <HeadlessUIModalComponent
                ...
                headingChildren={<span>Create New {currentClassName} Plan</span>}
            ...
            <HeadlessUIModalComponent
                ...
                headingChildren={<span>Delete {currentClassName} Class Plan</span>}
            ...
            <div className='flex items-center justify-between' >
                <h1 className='text-2xl'>{currentClassName} Class Plans</h1>
                <button
                ...
                >
                    <PlusIcon className='h-5 w-5' /> Add New {currentClassName} Plan
                </button>
            </div>
                ...
                <div>
                    {
                        yogaClassPlans.length === 0 ?
                            <p className='mt-10 text-xl text-gray-700'>No {currentClassName} Classes to display{yogaClassPlans.length ? ' with selected filter' : ''}!</p>
                            :
